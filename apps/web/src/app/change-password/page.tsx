...
export default function ChangePasswordPage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setNewPassword("");
      setConfirmPassword("");
      setMessage("密码已更新，正在进入系统...");
      router.replace(getPostLoginDestination(nextUser, null));
    } catch (error) {
  ...
  return (
    <AuthShell
      ...
      title="请先修改密码"
      description="为了满足线上使用要求，管理员初始化账号和批量生成的临时账号首次登录时都必须先完成密码更新。"
      asideTitle="先完成安全收口，再进入工作区。"
      asideDescription="改密通过后，系统会继续根据账号身份进入对应工作区；若你同时具备多个身份，下一步可以主动选择本次进入方式。"
      highlights={[
